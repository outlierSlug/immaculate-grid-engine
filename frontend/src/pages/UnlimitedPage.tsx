        ...
        <div className="flex justify-center">
          ...
          {!isGameOver && (
            <button
              ...
              // Icon-only below sm, same treatment (and same reason) as
              // Generate beside it: this row's per-cell grid column is
              // narrow at phone widths, and "Give Up" wrapped to two lines
              // there, making the button noticeably taller than the icon
              // buttons either side of it. A flag reads as "surrender"
              // without needing the words. It swaps *for* the label rather
              // than sitting beside it - at sm+ the icon pushed this
              // button wide enough to collide with Generate. Daily's own
              // Give Up button sits on its own full-width row, so it keeps
              // the plain label at every width.
              aria-label="Give Up"
              className="flex items-center justify-center p-2.5 sm:px-5 sm:py-2.5 rounded-full border border-red-300 dark:border-red-800/70 text-gray-600 dark:text-gray-400 font-semibold hover:bg-red-50 dark:hover:bg-red-950/30 hover:border-red-400 dark:hover:border-red-700 transition cursor-pointer"
            >
              <svg className="w-5 h-5 shrink-0 sm:hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2} aria-hidden="true">
