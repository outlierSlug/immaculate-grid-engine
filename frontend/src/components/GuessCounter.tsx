...
interface GuessCounterProps {
  ...
  // Distinguishes "stopped early" from "still going" - without this, the
  // tooltip reads "You have X guesses remaining" even after giving up,
  // which is misleading (the game is over; those guesses were never spent,
  // but they're not available either).
  gaveUp?: boolean;
}

export default function GuessCounter({ remaining, iconSrc, feedback, gaveUp }: GuessCounterProps) {
  const unlimited = remaining === null;
  const depleted = remaining === 0;
  const description = gaveUp
    ? unlimited
      ? 'You gave up.'
      : `You gave up with ${remaining} guess${remaining === 1 ? '' : 'es'} remaining.`
    : unlimited
    ? 'You have unlimited guesses remaining.'
    : `You have ${remaining} guess${remaining === 1 ? '' : 'es'} remaining.`;
  ...
  return (
    <ClickTooltip heading="Guesses Remaining" description={description}>
      <div className={depleted ? 'text-gray-500 dark:text-gray-500' : 'text-black dark:text-gray-100'}>
        <div
          key={popKey}
