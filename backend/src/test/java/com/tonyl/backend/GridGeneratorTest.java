...
class GridGeneratorTest {
    ...
    // ── Ad-hoc: Clash Royale's new "targeting" category (see
    // ingestion/clashroyale/raw/targeting_definitive_plan.txt) - same
    // methodology/structure as starRailAffiliationTuningReport above, just
    // scoped to a 3-value dimension instead of a 12-value one and with no
    // min-count floor (targeting's thinnest value already has 28 members).
    // Deliberately not a regression gate - a report to read before deciding
    // whether the default weight needs adjusting. ──
    @Test
    void clashRoyaleTargetingTuningReport() throws Exception {
        GameModule module = new ClashRoyaleGameModule();
        List<GridItem> entities = loadEntities("clashroyale_entities.json");
        List<CategoryDefinition> categories = module.getCategoryDefinitions(entities);
        ...
        int rowsMono = 0, colsMono = 0;
        Map<String, Integer> dimensionSlotAppearances = new TreeMap<>();
        Map<String, Integer> targetingValueGridAppearances = new TreeMap<>();
        Map<String, List<Integer>> depthsByDimension = new TreeMap<>();
        int globalThinnestCell = Integer.MAX_VALUE, globalWidestCell = Integer.MIN_VALUE;
        String globalThinnestCellLabel = null, globalWidestCellLabel = null;
        List<Integer> totalValidAnswersPerGrid = new ArrayList<>();
        ...
        for (long seed = 0; seed < sampleSize; seed++) {
            ...
            Set<String> targetingValuesThisGrid = new HashSet<>();
            Stream.concat(puzzle.rowCategories().stream(), puzzle.colCategories().stream())
                .filter(cat -> cat.getDimension().equals("targeting"))
                .forEach(cat -> targetingValuesThisGrid.add(cat.getLabel()));
            for (String value : targetingValuesThisGrid) targetingValueGridAppearances.merge(value, 1, Integer::sum);
        ...
        int totalSlots = successCount * 2 * GRID_SIZE;
        System.out.println();
        System.out.println("=== Clash Royale targeting tuning report (" + successCount + "/" + sampleSize + " successful) ===");
        System.out.println();
        System.out.println("-- Dimension distribution (share of all " + totalSlots + " row+col category slots) --");
        for (var entry : dimensionSlotAppearances.entrySet()) {
            System.out.printf("  %-14s %5d  (%.1f%%)%n", entry.getKey(), entry.getValue(), 100.0 * entry.getValue() / totalSlots);
        ...
        System.out.printf("Cols entirely one dimension: %d/%d (%.1f%%)%n", colsMono, successCount, 100.0 * colsMono / successCount);
        System.out.println();
        System.out.println("-- Targeting values: grid-appearance rate (% of grids where this value appears as a row or col category) --");
        int finalSuccessCount = successCount;
        targetingValueGridAppearances.entrySet().stream()
            .sorted((a, b) -> b.getValue() - a.getValue())
            .forEach(entry -> System.out.printf("  %-22s %5d / %5d  (%.1f%%)%n", entry.getKey(), entry.getValue(), finalSuccessCount, 100.0 * entry.getValue() / finalSuccessCount));
        System.out.println();
        System.out.println("-- Per-dimension cell depth --");
